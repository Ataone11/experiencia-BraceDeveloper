...
export class FilesService {
  ...
  async uploadImagen(baseRoute: string, file: Express.Multer.File) {
    const s3 = new S3();
    ...
    const res = await s3.upload({
      ...
      Bucket: process.env.AWS_S3_BUCKET_NAME,
      Body: dataBuffer,
      Key: `${baseRoute}/imagen.${formato}`
    }).promise()

    return res.Key;
  }

  async uploadBuffer(baseRoute: string, dataBuffer: Buffer) {
    const s3 = new S3();
    ...
    const res = await s3.upload({
      ...
      Bucket: process.env.AWS_S3_BUCKET_NAME,
      Body: dataBuffer,
      Key: `${baseRoute}/${moment().format("DD-MM-YYYY")}.jpg`
    }).promise()
